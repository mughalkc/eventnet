...
export default function CreateEvent() {
  ...
  const handleGenerateDescription = async () => {
    ...
    setAiLoading(true)
    try {
      const response = await fetch('https://eventnet-6c6d.vercel.app/api/vendor/ai/generate-description', {
        method: 'POST',
        headers: {
  ...
  const checkConflict = async () => {
    ...
    try {
      const response = await fetch('https://eventnet-6c6d.vercel.app/api/vendor/events/check-conflict', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    try {
      ...
      if (eventData.image) {
        ...
        const imageUrl = eventData.image.startsWith('http') 
          ? eventData.image 
          : `https://eventnet-6c6d.vercel.app/${eventData.image.replace(/^\//, '')}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        try {
          ...
          if (image) {
            ...
            response = await fetch(`https://eventnet-6c6d.vercel.app/api/vendor/events/${eventId}`, {
              method: 'PUT',
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              },
      ...
      const endpoint = 'https://eventnet-6c6d.vercel.app/api/vendor/events';
