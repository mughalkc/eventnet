...
const Vendors = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      // Fetch all vendors
      const response = await fetch('https://eventnet-6c6d.vercel.app/api/admin/vendors', {
        headers: {
  ...
  const fetchEventCounts = async (vendorIds) => {
    try {
      const response = await fetch('https://eventnet-6c6d.vercel.app/api/admin/vendors/event-counts', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('adminToken')}`,
  ...
  const handleStatusUpdate = async (vendor, newStatus) => {
    try {
      const response = await fetch(`https://eventnet-6c6d.vercel.app/api/admin/vendors/${vendor._id}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('adminToken')}`,
          'Content-Type': 'application/json'
